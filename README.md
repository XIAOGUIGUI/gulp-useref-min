# [gulp](https://github.com/gulpjs/gulp)-useref [![Build Status](https://travis-ci.org/jonkemp/gulp-useref-min.svg?branch=master)](https://travis-ci.org/jonkemp/gulp-useref-min) [![Coverage Status](https://coveralls.io/repos/jonkemp/gulp-useref-min/badge.svg?branch=master&service=github)](https://coveralls.io/github/jonkemp/gulp-useref-min?branch=master)

[![NPM](https://nodei.co/npm/gulp-useref-min.png?downloads=true)](https://nodei.co/npm/gulp-useref-min/)
...
Install with [npm](https://npmjs.org/package/gulp-useref-min)

```
npm install --save-dev gulp-useref-min
```
...
var gulp = require('gulp'),
    useRefMin = require('gulp-useref-min');
...
gulp.task('default', function () {
    return gulp.src('index.html')
        .pipe(useRefMin({}, lazypipe().pipe(sourcemaps.init, { loadMaps: true })))
...
Use additional streams as sources of assets. Useful for combining gulp-useref-min with preprocessing tools. For example, to use with TypeScript:
...
gulp.task('default', function () {
    // use gulp-useref-min normally
    return gulp.src('src/index.html')
        .pipe(useRefMin({ additionalStreams: [tsStream] }))
        .pipe(gulp.dest('dist'));
});
```
...
If you upgrade gulp-useref-min from v2 without changing your gulpfile,
...
whether they have upgraded to the V3 gulp-useref-min API, rather than
...
See the [CONTRIBUTING Guidelines](https://github.com/jonkemp/gulp-useref-min/blob/master/CONTRIBUTING.md)
